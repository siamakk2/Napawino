...
const SITEMAP_ADD = [
  ...
  ['first-tasting-guide.html', 'monthly', '0.9']
];
...
const LLMS_ADD =
  ...
  "- [What to Expect at Your First Napa Wine Tasting](https://www.napawino.com/first-tasting-guide.html): what actually happens at a tasting, what to wear, what to say, tipping norms, and reassurance that no expertise is required\n" +
...
(async () => {
  ...
  if (!fs.existsSync('source/article.css')) die('missing source/article.css');
  live['article.css'] = fs.readFileSync('source/article.css');
  console.log(`loaded article.css from source/ (${live['article.css'].length} B)`);
  ...
  const total = new Set(CARRY.concat(['data.js', 'index.html', 'site.css', 'article.css']).concat(NEW_PAGES)).size;
  const written = [];
  (function walk(d) {
